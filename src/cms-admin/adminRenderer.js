...
function buildMediaWorkspaceModel(state = {}) {
  ...
  const selectedAsset = mediaWorkspaceState.selectedAssetId
    ...
    : null;
  const summary = summarizeMediaLibrary(activeUploadAssets);
  summary.libraryFiles = activeUploadAssets.length;
  ...
  summary.contentReferences = virtualReferences.length;
  summary.matchedReferenceFiles = matchedReferenceAssets.length;
  summary.matchedReferenceLocations = matchedReferenceAssets.reduce((total, asset) => total + safeArray(asset.usage?.references).length, 0);
  summary.orphanReferences = orphanReferences.length;
  summary.brokenReference = brokenReferences.length;
  summary.needsContentFix = brokenReferences.length;
  return {
    state,
    mediaError,
    usageContext,
    ...
    virtualAssets,
    virtualReferences,
    matchedReferenceAssets,
    orphanReferences,
    brokenReferences,
    contentReferences: virtualReferences,
    usageAssets,
    allAssets,
    selectedAsset,
    summary,
...
function renderMediaLibraryWorkspace(model = {}) {
  const panel = createElement('section', { className: 'cms-admin-media-workspace-tab cms-admin-media-library-workspace cms-admin-media-library-panel' });
  panel.appendChild(renderMediaWorkspaceHeader(model, {
    title: 'Thư viện ảnh & video',
    subtitle: 'Các file thật có bản ghi upload trong CMS. Màn này chỉ xem, lọc, preview và kiểm tra; website public chưa đổi.',
    badge: 'File thật trong Thư viện',
  }));

  if (model.mediaError) {
    panel.appendChild(renderErrorBox(model.mediaError, 'Không đọc được danh sách ảnh/video'));
  }

  if (!safeArray(model.activeUploadAssets).length) {
    const empty = createElement('section', { className: 'cms-admin-panel cms-admin-view-panel cms-admin-media-empty-panel' });
    empty.appendChild(renderMediaLibraryEmptyState());
    panel.appendChild(empty);
  } else {
    panel.appendChild(renderMediaLibraryControls(model.activeUploadAssets));
    panel.appendChild(renderMediaProfessionalWorkspace(model, {
      mode: 'library',
      assets: model.activeUploadAssets,
      title: 'File thật trong Thư viện',
      subtitle: 'Chọn một file có bản ghi upload để xem preview, nơi đang dùng và thao tác an toàn ở rail bên phải.',
      emptyText: 'Không có ảnh/video phù hợp với bộ lọc hiện tại.',
    }));
  }
...
function renderMediaWorkspaceHeader(model = {}, options = {}) {
  const header = createElement('section', { className: 'cms-admin-panel cms-admin-view-panel cms-admin-media-command-header' });
  const top = createElement('div', { className: 'cms-admin-media-command-top' });
  const titleWrap = createElement('div');
  titleWrap.appendChild(createElement('span', { className: 'cms-admin-eyebrow', text: 'ẢNH & VIDEO' }));
  titleWrap.appendChild(createElement('h3', { className: 'cms-admin-section-title', text: options.title || 'Thư viện ảnh & video' }));
  titleWrap.appendChild(createElement('p', { className: 'cms-admin-help-text', text: options.subtitle || 'Màn này chỉ xem, lọc, preview và kiểm tra ảnh/video.' }));
  const badges = createElement('div', { className: 'cms-admin-media-command-badges' });
  badges.appendChild(renderBadge(options.badge || 'Chỉ xem', 'info'));
  badges.appendChild(renderBadge('Website public chưa đổi', 'success'));
  appendChildren(top, [titleWrap, badges]);
...
function renderMediaTechnicalReferenceDetails() {
  const technical = createElement('details', { className: 'cms-admin-media-technical-details cms-admin-media-library-technical-reference' });
  technical.appendChild(createElement('summary', { text: 'Thông tin kỹ thuật để đối chiếu' }));
  technical.appendChild(renderTechnicalKeyValueList(ADMIN_COPY.media.technicalRows));
  technical.appendChild(renderMediaOperatorHandoffNotes());
  return technical;
}
...
function renderMediaVirtualInventoryNotice(model = {}) {
  ...
  const text = virtualCount
    ? 'Ngoài bản ghi upload, hệ thống cũng hiển thị các ảnh/video được nội dung nhắc tới nhưng có thể chưa có bản ghi upload riêng. Mở tab Đang dùng để xem nơi dùng.'
...
function renderMediaCountClarityNotice(model = {}) {
  ...
  notice.appendChild(renderBadge('Tổng quan ảnh/video', 'info'));
  notice.appendChild(createElement('p', {
    text: `Ảnh/video đang dùng được: ${formatCount(model.activeUploadAssets?.length || 0)}. Bản ghi upload giữ truy vết: ${formatCount((model.activeUploadAssets?.length || 0) + (model.deletedUploadAssets?.length || 0))} bản ghi, trong đó ${formatCount(model.deletedUploadAssets?.length || 0)} đã xóa khỏi kho lưu trữ. Đường dẫn được nội dung nhắc tới: ${formatCount(model.virtualReferences?.length || 0)} vị trí đã scan. Chỉ thấy trong nội dung: ${formatCount(model.virtualAssets?.length || 0)} đường dẫn; ${formatCount((model.virtualAssets || []).filter(isBrokenDeletedMediaReference).length)} tham chiếu hỏng tới file đã xóa. Đây không phải kiểm kê toàn bộ bucket; một file có thể xuất hiện ở nhiều nơi dùng.`,
  }));
  return notice;
}
...
function renderMediaCapabilityPolicyPanel(model = {}) {
  ...
  [
    ...
    ['Xóa có guard', 'Chỉ ảnh/video đang dùng được, có bản ghi upload/đường dẫn hợp lệ, không còn nơi dùng trong website/bản nháp/CMS và nhập đúng câu xác nhận mới được xóa. File đã xóa chỉ còn bản ghi kiểm tra.'],
  ].forEach(([title, body]) => {
...
function renderMediaDeletePolicyList() {
  ...
  [
    'Không xóa ảnh/video chỉ thấy trong nội dung.',
    'Không xóa ảnh/video đang có nơi dùng ở website, bản nháp hoặc CMS.',
    'Không xóa khi thiếu bản ghi upload, kho lưu trữ/đường dẫn hoặc kiểm tra an toàn.',
    'Không chạy dọn/xóa hàng loạt từ màn Ảnh & video.',
...
function renderMediaCleanupWorkspace(model = {}) {
  ...
  panel.appendChild(renderMediaCleanupProgress(cleanupModel, selected));
...
function renderMediaCleanupProgress(cleanupModel = {}, candidate = null) {
  const decision = candidate ? cleanupModel.decisions?.[candidate.id] : null;
  const hasDecision = Boolean(decision);
  const hasPatch = Boolean(cleanupModel.applyResult?.ok);
  const isDirty = Boolean(cleanupModel.draftState?.dirty);
  const steps = [
    {
      label: 'Chọn mục cần kiểm tra',
      state: candidate ? 'done' : cleanupModel.hasDraft ? 'active' : 'blocked',
    },
    {
      label: 'Chọn cách xử lý',
      state: hasDecision ? 'done' : candidate ? 'active' : 'blocked',
    },
    {
      label: 'Cập nhật bản nháp',
  ...
  const wrap = createElement('ol', { className: 'cms-admin-media-cleanup-steps', attrs: { 'aria-label': 'Trình tự xử lý đường dẫn trong bản nháp' } });
  steps.forEach((step, index) => {
    const item = createElement('li', { className: `cms-admin-media-cleanup-step is-${step.state}` });
    item.appendChild(createElement('span', { className: 'cms-admin-media-cleanup-step-index', text: String(index + 1) }));
    item.appendChild(createElement('span', { className: 'cms-admin-media-cleanup-step-label', text: step.label }));
    item.appendChild(createElement('small', { text: step.state === 'done' ? 'Đã xong' : step.state === 'active' ? 'Đang thực hiện' : 'Chưa đến bước' }));
...
function getMediaCleanupOwner(tokens = []) {
  ...
  if (tokens[0] === 'site_settings' || tokens[0] === 'site') {
    return { type: 'site', id: String(tokens[0]), label: 'Thông tin website' };
  }
...
function renderMediaCleanupHeader(cleanupModel = {}) {
  ...
  header.appendChild(top);
  const remaining = Math.max(0, Number(cleanupModel.summary?.needsReview || 0) - Number(cleanupModel.summary?.selectedCount || 0));
  const stats = createElement('div', { className: 'cms-admin-media-command-stats cms-admin-media-command-stats-compact' });
  [
    ['Tổng đường dẫn trong bản nháp', cleanupModel.summary?.draftReferences ?? 0],
    ['Cần kiểm tra', cleanupModel.summary?.needsReview ?? 0],
    ['Đã chọn cách xử lý', cleanupModel.summary?.selectedCount ?? 0],
    ['Còn lại', remaining],
  ].forEach(([label, value]) => {
    const tile = createElement('div', { className: 'cms-admin-media-command-stat' });
    tile.appendChild(createElement('span', { text: label }));
  ...
  header.appendChild(stats);
  const validationLine = createElement('p', { className: 'cms-admin-media-cleanup-validation-summary' });
  const validationText = cleanupModel.summary?.validationStatus === 'not_run'
    ? 'Kiểm tra bản nháp — Chưa chạy.'
    : cleanupModel.summary?.validationStatus === 'valid'
      ? 'Kiểm tra bản nháp — Không thấy lỗi.'
      : `Kiểm tra bản nháp — ${formatCount(cleanupModel.summary?.validationErrors || 0)} lỗi cần sửa.`;
  validationLine.textContent = validationText;
  header.appendChild(validationLine);
  return header;
}
...
function renderMediaCleanupNoCandidatePanel(cleanupModel = {}) {
  ...
  card.appendChild(createElement('p', { text: 'Các đường dẫn trong bản nháp đã khớp file Thư viện hoặc chưa phát hiện mục cần xử lý theo quy tắc hiện tại.' }));
  if (cleanupModel.validationStatus === 'invalid') {
    card.appendChild(renderCompactNotice(`Kiểm tra bản nháp còn ${formatCount(cleanupModel.validationErrors || 0)} lỗi thật. Hãy xử lý ở màn Nội dung phòng 3D trước khi lưu hoặc công khai.`));
  } else {
    card.appendChild(renderCompactNotice('Kiểm tra bản nháp đang đạt theo quy tắc hiện có. Website đang công khai chưa đổi.'));
  }
  return card;
}
...
function renderMediaCleanupCandidateList(cleanupModel = {}, selected = null) {
  ...
  card.appendChild(renderPanelTitle('Ảnh/video cần đối chiếu', `${formatCount(cleanupModel.candidates.length)} mục`));
  card.appendChild(createElement('p', { className: 'cms-admin-help-text', text: 'Chọn từng mục để xem nơi đang dùng, mức cần chú ý và chọn cách xử lý.' }));
  ...
  safeArray(cleanupModel.candidates).forEach((candidate) => {
    ...
    button.appendChild(createElement('strong', { text: getMediaCleanupBusinessTitle(candidate) }));
    button.appendChild(createElement('span', { text: getMediaCleanupBusinessSubtitle(candidate) }));
    const badges = createElement('div', { className: 'cms-admin-media-cleanup-candidate-badges' });
    if (active) badges.appendChild(renderBadge('Đang chọn', 'info'));
    ...
    badges.appendChild(renderBadge(getMediaCleanupRiskLabel(candidate), candidate.riskLevel === 'Cao' ? 'warning' : 'default'));
...
function renderMediaCleanupInspector(cleanupModel = {}, candidate = null) {
  ...
  card.appendChild(renderPanelTitle('Thông tin mục đang chọn', candidate ? getMediaCleanupBusinessTitle(candidate) : 'Chưa chọn'));
  if (!candidate) {
    card.appendChild(renderEmptyState('Chọn một mục ở danh sách bên trái để xem chi tiết.'));
    return card;
  }
  card.appendChild(createElement('p', { className: 'cms-admin-help-text', text: getMediaCleanupBusinessReason(candidate) }));
  const grid = createElement('dl', { className: 'cms-admin-media-cleanup-detail-grid' });
  [
    ['Nơi đang dùng', getMediaCleanupOwnerDisplayLabel(candidate)],
    ['Vị trí dữ liệu', getMediaCleanupFieldDisplayName(candidate)],
    ['Trạng thái', getMediaCleanupClassificationLabel(candidate.classification)],
    ['Mức cần chú ý', getMediaCleanupRiskLabel(candidate)],
  ...
  if (candidate.normalizedValue) {
    ...
    preview.appendChild(createElement('p', { className: 'cms-admin-help-text', text: 'Đây là ảnh/video mà bản nháp hiện đang nhắc tới. Thay đổi ở bước này chưa làm website đang công khai thay đổi.' }));
    preview.appendChild(renderMediaPreview(previewAsset));
    card.appendChild(preview);
  } else {
    card.appendChild(renderCompactNotice('Không thể xem trước ảnh/video này. Bạn vẫn có thể kiểm tra vị trí dữ liệu và chọn file thay thế.'));
  }
  ...
  [
    ...
    ['Field name', candidate.fieldName || '—'],
    ['Đường dẫn hiện tại đầy đủ', candidate.currentValue],
...
function renderMediaCleanupReplacementPicker(cleanupModel = {}, candidate = {}) {
  ...
  wrap.appendChild(createElement('h4', { text: 'Chọn file thay thế' }));
  wrap.appendChild(createElement('p', { className: 'cms-admin-help-text', text: 'Chỉ hiển thị ảnh/video đang có trong Thư viện và phù hợp với loại nội dung này. Chọn file chưa làm thay đổi bản nháp.' }));
  ...
  if (!candidate.canReplace) {
    wrap.appendChild(renderCompactNotice('Mục này chưa xác định được loại ảnh/video phù hợp nên chưa cho thay tự động.'));
    return wrap;
  }
  if (!compatible.length) {
    wrap.appendChild(renderCompactNotice('Chưa có ảnh/video phù hợp trong Thư viện. Hãy bổ sung file ở luồng quản lý nội dung phù hợp rồi quay lại màn này.'));
    return wrap;
  }
  ...
  if (selectedAsset) {
    ...
    preview.appendChild(createElement('p', { className: 'cms-admin-help-text', text: `${getMediaDisplayName(selectedAsset)} — ${selectedAsset.mediaKind === 'video' ? 'Video' : 'Ảnh'}. Chưa cập nhật bản nháp.` }));
...
function renderMediaCleanupActionRail(cleanupModel = {}, candidate = null) {
  ...
  const checklist = createElement('div', { className: 'cms-admin-media-checklist cms-admin-media-cleanup-short-checklist' });
  buildMediaCleanupChecklist(cleanupModel, candidate).forEach((item) => checklist.appendChild(renderMediaChecklistItem(item)));
  rail.appendChild(checklist);

  if (!cleanupModel.hasDraft) {
    const draftState = cleanupModel.draftState || {};
    const actions = createElement('div', { className: 'cms-admin-media-cleanup-actions cms-admin-media-cleanup-action-group' });
    actions.appendChild(createElement('h4', { text: 'Bắt đầu kiểm tra' }));
    const loadButton = createElement('button', {
      className: 'cms-admin-button cms-admin-button-primary',
      text: draftState.loading ? 'Đang tải nội dung...' : draftState.loadError ? 'Thử tải lại' : 'Tải nội dung đang công khai',
      type: 'button',
  ...
  if (candidate) {
    const decision = cleanupModel.decisions?.[candidate.id];
    const hasReplacement = Boolean(mediaWorkspaceState.cleanupSelectedReplacementId);
    const decisionGroup = createElement('section', { className: 'cms-admin-media-cleanup-action-group' });
    decisionGroup.appendChild(createElement('h4', { text: '1. Quyết định cho mục đang chọn' }));
    const replaceButton = createElement('button', { className: 'cms-admin-button cms-admin-button-primary', text: 'Chọn file này để thay thế', type: 'button' });
    replaceButton.disabled = !hasReplacement;
    ...
    decisionGroup.appendChild(replaceButton);
    if (!hasReplacement) decisionGroup.appendChild(createElement('p', { className: 'cms-admin-help-text', text: 'Chọn file thay thế ở cột giữa trước khi ghi nhận phương án thay thế.' }));
    ...
    const skipButton = createElement('button', { className: 'cms-admin-button cms-admin-button-ghost', text: 'Bỏ qua lần này', type: 'button' });
    skipButton.addEventListener('click', () => setMediaCleanupDecision(candidate.id, { action: 'skip' }));
    decisionGroup.appendChild(keepButton);
    decisionGroup.appendChild(skipButton);
    if (decision) decisionGroup.appendChild(createElement('p', { className: 'cms-admin-media-cleanup-action-feedback', text: 'Đã ghi nhận phương án. Bản nháp chưa được cập nhật.' }));
    rail.appendChild(decisionGroup);
  }

  const applyGroup = createElement('section', { className: 'cms-admin-media-cleanup-action-group' });
  applyGroup.appendChild(createElement('h4', { text: '2. Cập nhật bản nháp' }));
  applyGroup.appendChild(createElement('p', { className: 'cms-admin-help-text', text: 'Thao tác này chỉ cập nhật bản nháp đang mở trong trình duyệt. Chưa lưu lên CMS và chưa làm website đang công khai thay đổi.' }));
  const applyButton = createElement('button', { className: 'cms-admin-button cms-admin-button-primary', text: 'Cập nhật bản nháp trong trình duyệt', type: 'button' });
  ...
  if (!hasMediaCleanupPatchDecision(cleanupModel)) applyGroup.appendChild(createElement('p', { className: 'cms-admin-help-text', text: 'Cần có ít nhất một mục đã chọn file thay thế trước khi cập nhật bản nháp.' }));
  if (cleanupModel.applyResult) applyGroup.appendChild(renderMediaCleanupApplyResult(cleanupModel.applyResult));
  ...
  if (!cleanupModel.draftState?.dirty) saveGroup.appendChild(createElement('p', { className: 'cms-admin-help-text', text: 'Chỉ lưu sau khi đã cập nhật bản nháp trong trình duyệt.' }));
  rail.appendChild(saveGroup);

  const resetDetails = createElement('details', { className: 'cms-admin-media-cleanup-reset-details' });
  resetDetails.appendChild(createElement('summary', { text: 'Tùy chọn khác' }));
  resetDetails.appendChild(createElement('p', { className: 'cms-admin-help-text', text: 'Đặt lại sẽ xóa toàn bộ thay đổi chưa lưu trong bản nháp đang mở, không chỉ mục hiện tại.' }));
  const resetButton = createElement('button', { className: 'cms-admin-button', text: 'Đặt lại toàn bộ bản nháp đã tải', type: 'button' });
...
function buildMediaCleanupChecklist(cleanupModel = {}, candidate = null) {
  if (!cleanupModel.hasDraft) {
    ...
    return [
      { label: 'Bản nháp CMS', value: loading ? 'Đang tải' : 'Chưa có', tone: 'neutral', detail: loading ? 'Đang đọc nội dung hiện tại.' : 'Cần tải nội dung trước khi quét đường dẫn.' },
      { label: 'Kiểm tra đường dẫn', value: 'Chưa chạy', tone: 'neutral', detail: 'Chưa có bản nháp nên chưa quét.' },
      { label: 'Website đang công khai', value: 'Chưa đổi', pass: true, detail: 'Mở tab và tải nội dung không công khai website.' },
      { label: 'Kho file', value: 'Không thay đổi', pass: true, detail: 'Không upload, không xóa file.' },
    ];
  }

  if (!candidate && !cleanupModel.candidates.length) {
    return [
      { label: 'Bản nháp CMS', value: 'Đã tải', pass: true, detail: 'Đang kiểm tra bản nháp đang mở.' },
      { label: 'Quét đường dẫn', value: 'Đã chạy', pass: true, detail: 'Không phát hiện mục cần xử lý theo rule hiện tại.' },
      { label: 'Đường dẫn cần xử lý', value: 'Không có', pass: true, detail: 'Không có mục cần review trong bản nháp.' },
      { label: 'Kiểm tra bản nháp', value: cleanupModel.validationStatus === 'valid' ? 'Không thấy lỗi' : `${formatCount(cleanupModel.validationErrors || 0)} lỗi`, pass: cleanupModel.validationStatus === 'valid', detail: cleanupModel.validationStatus === 'valid' ? 'Bản nháp đạt kiểm tra hiện có.' : 'Đây là lỗi thật của bản nháp đã tải.' },
      { label: 'Website đang công khai', value: 'Chưa đổi', pass: true, detail: 'Chưa publish trong workflow này.' },
    ];
  }
  ...
  return [
    { label: 'Mục đã chọn', value: candidate ? 'Có' : 'Chưa chọn', pass: Boolean(candidate), detail: candidate ? getMediaCleanupBusinessTitle(candidate) : 'Chọn một mục để xử lý.' },
...
function applyMediaCleanupDraftDecisions(cleanupModel = {}) {
  ...
  if (!changed.length) {
    mediaWorkspaceState.cleanupApplyResult = { ok: false, message: 'Chưa có mục nào chọn thay file để cập nhật bản nháp.' };
    renderAdminShell();
    return;
  }
  const validation = validateStaticCmsDraft(nextDraft, STATIC_CMS_DRAFT_CONFIG);
  if (!validation.valid) {
    mediaWorkspaceState.cleanupApplyResult = { ok: false, message: 'Bản nháp sau khi cập nhật chưa đạt kiểm tra. Chưa áp dụng thay đổi.', validation };
    renderAdminShell();
    return;
  }
...
function resetMediaCleanupDraft(cleanupModel = {}) {
  const ok = window.confirm('Đặt lại toàn bộ bản nháp đã tải? Thao tác này xóa toàn bộ thay đổi chưa lưu trong bản nháp đang mở. Website public không thay đổi.');
  if (!ok) return;
...
function renderMediaCleanupApplyResult(result = {}) {
  ...
  box.appendChild(renderBadge(result.ok ? 'Đã cập nhật bản nháp' : 'Cần kiểm tra', result.ok ? 'success' : 'warning'));
...
function renderMediaCleanupSaveConfirmModal(cleanupModel = {}) {
  ...
  [
    ...
    `${formatCount(cleanupModel.applyResult?.changedCount || 0)} vị trí dữ liệu đã được thay đổi trong bản nháp trong trình duyệt.`,
    `${formatCount(Object.values(cleanupModel.decisions || {}).filter((decision) => decision?.action === 'skip').length)} mục đã được bỏ qua lần này.`,
  ].forEach((item) => list.appendChild(createElement('li', { text: item })));
...
function renderMediaCleanupDiagnostics(cleanupModel = {}) {
  ...
  [
    ...
    ['File thay thế hợp lệ', `${formatCount(cleanupModel.activeLibraryFiles?.length || 0)} file trong Thư viện`],
    ['Mục cần review', formatCount(cleanupModel.candidates?.length || 0)],
...
function getMediaCleanupBusinessTitle(candidate = {}) {
  const field = String(candidate.fieldName || '').toLowerCase();
  if (candidate.ownerType === 'site' && (field.includes('logo') || String(candidate.jsonPathLabel || '').toLowerCase().includes('logo'))) return 'Logo cũ trong Thông tin website';
  if (candidate.ownerType === 'featured') return 'Ảnh/video trong Tác phẩm tiêu biểu';
  if (candidate.ownerType === 'room') return `${getMediaCleanupFieldDisplayName(candidate)} trong ${getMediaCleanupOwnerDisplayLabel(candidate)}`;
  return getMediaCleanupFieldDisplayName(candidate) || 'Ảnh/video trong bản nháp';
}

function getMediaCleanupBusinessSubtitle(candidate = {}) {
  const status = getMediaCleanupClassificationLabel(candidate.classification);
  const owner = getMediaCleanupOwnerDisplayLabel(candidate);
  return `${owner} · ${status}`;
}

function getMediaCleanupBusinessReason(candidate = {}) {
  if (candidate.ownerType === 'site' && String(candidate.fieldName || '').toLowerCase().includes('logo')) return 'Đường dẫn ảnh logo cũ cần đối chiếu trước khi thay đổi.';
  if (candidate.classification === 'legacy_keep_candidate') return 'Đường dẫn cũ hoặc local cần review thủ công trước khi thay bằng file trong Thư viện.';
  if (candidate.classification === 'unsafe_reference') return 'Đường dẫn hiện tại không đạt kiểm tra an toàn, cần chọn file thay thế nếu phù hợp.';
  if (candidate.classification === 'broken_reference') return 'Nội dung đang trỏ tới ảnh/video không còn dùng được, cần chọn file thay thế.';
  return candidate.reason || 'Đường dẫn trong bản nháp cần được đối chiếu với file trong Thư viện.';
}

function getMediaCleanupOwnerDisplayLabel(candidate = {}) {
  if (candidate.ownerType === 'site') return 'Thông tin website';
  if (candidate.ownerType === 'featured') return 'Tác phẩm tiêu biểu trên Trang chủ';
  return candidate.ownerLabel || 'Bản nháp CMS';
}

function getMediaCleanupFieldDisplayName(candidate = {}) {
  const field = String(candidate.fieldName || '').toLowerCase();
  const path = String(candidate.jsonPathLabel || '').toLowerCase();
  if (field.includes('logo') || path.includes('logo')) return 'Logo';
  if (field.includes('poster')) return 'Ảnh poster';
  if (field.includes('thumbnail') || field.includes('thumb')) return 'Ảnh đại diện';
  if (field.includes('video')) return 'Video';
  if (field.includes('image') || field.includes('img') || field.includes('photo') || field.includes('media')) return 'Ảnh/video';
  return formatMediaFieldName(candidate.fieldName || '') || 'Vị trí dữ liệu';
}

function getMediaCleanupRiskLabel(candidate = {}) {
  return candidate.riskLevel === 'Cao' ? 'Cần kiểm tra kỹ' : `Rủi ro ${candidate.riskLevel || 'Trung bình'}`;
}

function getMediaCleanupFieldLabel(candidate = {}) {
  return `${getMediaCleanupFieldDisplayName(candidate)} — ${getMediaCleanupOwnerDisplayLabel(candidate)}`;
}
...
function renderMediaProfessionalWorkspace(model = {}, options = {}) {
  ...
  workspace.appendChild(renderMediaMasterListCard(assets, {
    title: options.title || 'Danh sách ảnh/video',
    subtitle: options.subtitle || 'Chọn ảnh/video để xem thông tin và thao tác an toàn.',
    emptyText: options.emptyText || ADMIN_COPY.media.emptyFiltered,
  }));
  workspace.appendChild(renderMediaInspectorCard(selected));
  workspace.appendChild(renderMediaActionSafetyCard(selected, model, options));
  return workspace;
}
...
function renderMediaMasterListItem(asset = {}) {
  ...
  copy.appendChild(createElement('strong', { text: getMediaDisplayName(asset) }));
  const semantic = getMediaSemanticState(asset);
  ...
  badges.appendChild(renderBadge(semantic.label, semantic.variant || 'default'));
  ...
  const meta = createElement('p', {
    ...
    text: `${semantic.shortNote} · ${getMediaTargetLabel(asset)} · ${asset.roomKey ? getRoomLabel(asset.roomKey) : 'Không rõ phòng'}`,
  });
...
function renderMediaInspectorCard(asset = null) {
  ...
  card.appendChild(renderPanelTitle('Thông tin ảnh/video', asset ? 'Đang xem mục đã chọn' : 'Chọn ảnh/video'));
  if (!asset) {
    card.appendChild(renderEmptyState('Chọn một ảnh/video hoặc đường dẫn ở danh sách để xem thông tin.'));
    return card;
  }
  card.appendChild(createElement('p', {
    className: 'cms-admin-help-text cms-admin-media-inspector-context',
    text: 'Thông tin gọn cho ảnh/video đang chọn. Tên file, đường dẫn dài và metadata kỹ thuật nằm trong tab Tham chiếu & kỹ thuật.',
  }));
...
function renderMediaInspectorOverview(asset = {}) {
  ...
  [
    ['Loại ảnh/video', getMediaKindLabel(asset.mediaKind)],
    ...
    ['Nội dung', asset.itemId || asset.artworkCode || '—'],
    ['Vị trí ảnh/video', formatMediaFieldName(asset.fieldName)],
    ['Nơi dùng đã kiểm tra', asset.usage?.label || ADMIN_COPY.media.usageLabels.insufficient],
    ['Nơi đang dùng', asset.ownerLabel || getMediaTargetLabel(asset)],
    ['Nguồn dữ liệu', getMediaSemanticState(asset).label],
    ['Trạng thái ảnh/video', getMediaLifecycleState(asset).label],
  ].forEach(([label, value]) => appendMediaDetail(details, label, value));
...
function renderMediaInspectorTechnical(asset = {}) {
  ...
  const path = createElement('details', { className: 'cms-admin-media-technical-details cms-admin-media-path-details' });
  ...
  path.appendChild(renderTechnicalKeyValueList([
    ...
    ['Đường dẫn kỹ thuật', asset.publicUrl || asset.publicUrlRaw || asset.storagePath || '—'],
  ]));
  ...
  technical.appendChild(renderTechnicalKeyValueList([
    ...
    ['Khu vực dùng ảnh/video', asset.targetType || '—'],
    ['Vị trí kỹ thuật', asset.fieldName || '—'],
    ['Mã nguồn kỹ thuật', asset.sourceKey || '—'],
    ['Loại dữ liệu', getMediaSemanticState(asset).label],
  ]));
  wrap.appendChild(technical);
  return wrap;
}

function renderMediaActionSafetyCard(asset = null, model = {}, options = {}) {
  const card = createElement('aside', { className: 'cms-admin-panel cms-admin-view-panel cms-admin-media-action-card cms-admin-media-action-rail', attrs: { 'aria-label': 'Checklist và thao tác ảnh/video' } });
  const header = createElement('div', { className: 'cms-admin-media-rail-header' });
  const titleWrap = createElement('div');
  titleWrap.appendChild(createElement('span', { className: 'cms-admin-eyebrow', text: 'ẢNH & VIDEO' }));
  titleWrap.appendChild(createElement('h3', { className: 'cms-admin-panel-title', text: 'Checklist & thao tác' }));
  titleWrap.appendChild(createElement('p', { className: 'cms-admin-help-text', text: asset ? getMediaSemanticState(asset).label : 'Chọn một file hoặc đường dẫn để xem trạng thái và thao tác phù hợp.' }));
  header.appendChild(titleWrap);
  header.appendChild(renderBadge(options.mode === 'usage' ? 'Đang dùng' : 'Thư viện', 'info'));
  card.appendChild(header);

  if (!asset) {
    card.appendChild(renderEmptyState('Chọn ảnh/video để sao chép đường dẫn, mở nơi đang dùng hoặc kiểm tra điều kiện xóa.'));
    return card;
  }

  const context = createElement('div', { className: 'cms-admin-media-rail-context' });
  const semantic = getMediaSemanticState(asset);
  context.appendChild(createElement('span', { text: semantic.contextLabel }));
  context.appendChild(createElement('strong', { text: getMediaDisplayName(asset) }));
  context.appendChild(createElement('p', { text: getMediaOperatorStatusText(asset) }));
  card.appendChild(context);

  const checklist = createElement('div', { className: 'cms-admin-media-checklist' });
  buildMediaChecklistItems(asset, model).forEach((item) => checklist.appendChild(renderMediaChecklistItem(item)));
  card.appendChild(checklist);

  const actions = createElement('div', { className: 'cms-admin-media-action-buttons cms-admin-media-rail-actions' });
  ...
  if (!actions.childNodes.length) {
    actions.appendChild(createElement('span', { className: 'cms-admin-help-text', text: 'Chưa có đường dẫn an toàn hoặc nơi dùng rõ để thao tác nhanh.' }));
  }
  card.appendChild(actions);

  if (canShowMediaDeleteGuard(asset)) {
    card.appendChild(renderMediaDeleteGuard(asset, { actionPanel: true }));
    card.appendChild(renderMediaLockedDeleteSection(asset));
  } else {
    card.appendChild(renderMediaReferenceOnlyActionNotice(asset));
  }
  card.appendChild(createElement('p', { className: 'cms-admin-help-text cms-admin-media-public-note', text: 'Website public chưa đổi khi bạn chỉ xem, lọc, preview, sao chép đường dẫn hoặc mở nơi dùng.' }));
  return card;
}

function getMediaDisplayName(asset = {}) {
  return asset.caption || asset.title || asset.ownerLabel || asset.fileName || getShortMediaPath(asset.storagePath || asset.publicUrlRaw || asset.publicUrl) || 'Ảnh/video';
}

function getShortMediaPath(value = '') {
  const raw = String(value || '').split('?')[0];
  const pieces = raw.split('/').filter(Boolean);
  return pieces.slice(-2).join('/') || raw || '';
}

function getMediaSemanticState(asset = {}) {
  if (isDeletedMediaAsset(asset)) {
    return {
      key: 'deleted-file',
      label: 'Đã xóa khỏi kho lưu trữ',
      variant: 'danger',
      contextLabel: 'Bản ghi kiểm tra',
      shortNote: 'File đã xóa, chỉ còn audit log',
    };
  }
  if (isBrokenDeletedMediaReference(asset)) {
    return {
      ...
      label: 'Tham chiếu hỏng',
      variant: 'danger',
      contextLabel: 'Đường dẫn hỏng trong nội dung',
      shortNote: 'Cần mở nơi dùng để thay ảnh/video',
    };
  }
  if (asset.hasUploadLogRecord) {
    const refs = safeArray(asset.usage?.references);
    return {
      key: refs.length ? 'matched-library-file' : 'library-file',
      label: refs.length ? 'Khớp file trong Thư viện' : 'File thật trong Thư viện',
      variant: 'success',
      contextLabel: 'File thật trong Thư viện',
      shortNote: refs.length ? `${formatCount(refs.length)} vị trí đang dùng` : 'Có bản ghi upload trong CMS',
    };
  }
  const bucket = inferMediaSourceBucket(asset.publicUrlRaw || asset.storagePath || asset.publicUrl || '');
  const isExternal = bucket === 'External/R2';
  return {
    key: isExternal ? 'external-reference' : 'orphan-reference',
    label: isExternal ? 'Đường dẫn ngoài / chưa xác minh' : 'Chưa khớp file trong Thư viện',
    variant: 'warning',
    contextLabel: 'Đường dẫn được nội dung nhắc tới',
    shortNote: 'Không có bản ghi upload trong Thư viện',
  };
}

function canShowMediaDeleteGuard(asset = {}) {
  return Boolean(asset?.hasUploadLogRecord && !isDeletedMediaAsset(asset) && !isBrokenDeletedMediaReference(asset));
}

function renderMediaReferenceOnlyActionNotice(asset = {}) {
  const semantic = getMediaSemanticState(asset);
  const section = createElement('section', { className: `cms-admin-media-reference-only-notice cms-admin-media-reference-only-notice-${semantic.key}` });
  section.appendChild(createElement('h3', { text: semantic.key === 'broken-reference' ? 'Cần sửa nơi dùng' : 'Không xóa tại Media Library' }));
  const text = semantic.key === 'broken-reference'
    ? 'Nội dung đang trỏ tới file không còn dùng được. Cần mở nơi dùng để thay ảnh/video, không phải xóa file tại đây.'
    : 'Không thể xóa ở đây vì CMS chưa có bản ghi upload cho đường dẫn này. Đây là đường dẫn được nội dung nhắc tới, chưa được xác minh là file thật trong Thư viện.';
  section.appendChild(createElement('p', { text }));
  return section;
}

function getMediaOperatorStatusText(asset = {}) {
  const semantic = getMediaSemanticState(asset);
  if (semantic.key === 'deleted-file') return 'File đã xóa khỏi kho lưu trữ; chỉ còn bản ghi kiểm tra.';
  if (semantic.key === 'broken-reference') return 'Tham chiếu hỏng: nội dung đang trỏ tới file đã xóa. Hãy mở nơi dùng để thay ảnh/video.';
  if (semantic.key === 'orphan-reference' || semantic.key === 'external-reference') return 'Chỉ thấy đường dẫn trong nội dung; chưa khớp bản ghi upload nên không thể xóa tại Media Library.';
  if (!asset.hasSafePublicUrl) return asset.publicUrlSafetyMessage || 'Chưa có đường dẫn an toàn để preview/copy.';
  if (safeArray(asset.usage?.references).length) return 'File thật trong Thư viện đang được nội dung nhắc tới trong dữ liệu đã kiểm tra.';
  return 'File thật trong Thư viện, chưa thấy nơi dùng trong dữ liệu đã kiểm tra.';
}

function buildMediaChecklistItems(asset = {}, model = {}) {
  const refs = safeArray(asset.usage?.references);
  const lifecycle = getMediaLifecycleState(asset);
  const mediaError = model.mediaError;
  return [
    {
      label: 'Dữ liệu ảnh/video',
      pass: !mediaError,
      value: mediaError ? 'Có lỗi đọc' : 'Đã đọc',
      detail: mediaError ? normalizeErrorMessage(mediaError) : 'Danh sách ảnh/video đã được tải từ dữ liệu hiện có.',
    },
    {
      label: 'Ảnh/video đang chọn',
      pass: Boolean(asset?.id),
      value: asset?.id ? 'Có' : 'Chưa chọn',
      detail: asset?.id ? getMediaDisplayName(asset) : 'Chọn một ảnh/video trong danh sách.',
    },
    {
      label: 'Loại ảnh/video',
      pass: asset.mediaKind !== 'unknown',
      value: getMediaKindLabel(asset.mediaKind),
      detail: asset.mediaKind === 'unknown' ? 'Chưa xác định rõ là ảnh hay video.' : 'Loại ảnh/video đã được nhận diện từ dữ liệu hiện có.',
    },
    {
      label: 'Đường dẫn an toàn',
      pass: Boolean(asset.hasSafePublicUrl),
      value: asset.hasSafePublicUrl ? 'Đạt' : 'Không dùng được',
      detail: asset.hasSafePublicUrl ? 'Đường dẫn thuộc nguồn được phép.' : (asset.publicUrlSafetyMessage || 'Không có URL public an toàn.'),
    },
    {
      label: 'Preview',
      pass: Boolean(asset.hasSafePublicUrl),
      value: asset.hasSafePublicUrl ? 'Có thể preview' : 'Không có URL an toàn',
      detail: asset.hasSafePublicUrl ? 'Preview dùng đường dẫn đã qua kiểm tra an toàn.' : 'Không render preview nếu đường dẫn không an toàn.',
    },
    {
      label: 'Đang dùng ở',
      pass: refs.length > 0,
      value: refs.length ? `${formatCount(refs.length)} nơi` : 'Chưa thấy',
      detail: refs.length ? 'Có nơi dùng trong public/draft/CMS đã kiểm tra.' : 'Chưa thấy nơi dùng không đồng nghĩa an toàn để xóa.',
    },
    {
      label: 'Trạng thái ảnh/video',
      pass: lifecycle.key === 'active' && !isBrokenDeletedMediaReference(asset),
      value: getMediaSemanticState(asset).label,
      detail: getMediaOperatorStatusText(asset),
    },
    canShowMediaDeleteGuard(asset)
      ? {
        label: 'Xóa file',
        pass: refs.length === 0,
        value: refs.length ? 'Không xóa khi còn đang dùng' : 'Chỉ kiểm tra điều kiện',
        detail: 'Chỉ file thật trong Thư viện mới được kiểm tra điều kiện xóa. Server vẫn revalidate trước khi xóa thật.',
      }
      : {
        label: 'Cách xử lý',
        pass: !isBrokenDeletedMediaReference(asset),
        value: isBrokenDeletedMediaReference(asset) ? 'Mở nơi dùng để thay ảnh/video' : 'Không xóa tại Media Library',
        detail: getMediaOperatorStatusText(asset),
      },
    {
      label: 'Website public',
      pass: true,
      value: 'Chưa đổi',
      detail: 'Xem, lọc, preview và copy đường dẫn không làm thay đổi website.',
    },
  ];
}
...
function renderMediaChecklistItem(item = {}) {
  ...
  body.appendChild(createElement('span', { text: item.value || '—' }));
...
function renderMediaLockedDeleteSection(asset = {}) {
  ...
  section.appendChild(createElement('h3', { text: lifecycle.key === 'deleted' ? 'Ảnh/video đã xóa' : lifecycle.key === 'broken-reference' ? 'Tham chiếu hỏng' : 'Xóa ảnh/video' }));
  section.appendChild(createElement('p', {
    text: lifecycle.key === 'deleted'
      ...
      : lifecycle.key === 'broken-reference'
        ? 'Nội dung vẫn trỏ tới đường dẫn đã xóa. Hãy cập nhật nội dung liên quan, không xóa thêm tại màn Ảnh & video.'
  ...
  const button = createElement('button', {
    ...
    text: lifecycle.key === 'active' ? 'Xóa ảnh/video — khóa đến khi đủ điều kiện' : 'Không có thao tác xóa',
    type: 'button',
...
function renderMediaSelectedDetailPanel(selected = null, options = {}) {
  ...
  [
    ...
    ['Nơi dùng', selected.ownerLabel || getMediaTargetLabel(selected)],
    ['Loại dữ liệu', getMediaSemanticState(selected).label],
  ].forEach(([label, value]) => appendMediaDetail(details, label, value));
  ...
  technical.appendChild(renderTechnicalKeyValueList([
    ...
    ['Khu vực dùng ảnh/video', selected.targetType || '—'],
    ['Field', selected.fieldName || '—'],
    ['Mã nguồn kỹ thuật', selected.sourceKey || '—'],
  ]));
...
function buildVirtualMediaAssets(references = [], uploadAssets = [], deletedUploadAssets = []) {
  ...
  return Array.from(grouped.entries()).map(([key, group], index) => {
    ...
    return {
      ...
      sourceLabel: group.isBrokenDeletedReference ? 'Tham chiếu hỏng trong nội dung' : 'Đường dẫn trong nội dung chưa khớp file Thư viện',
...
function getMediaSourceLabel(asset = {}) {
  ...
  return getMediaSemanticState(asset).label;
}
...
function renderMediaUsageReferenceCard(asset = {}, reference = null) {
  ...
  header.appendChild(createElement('h4', { text: getMediaDisplayName(asset) }));
  const lifecycleBadge = renderMediaLifecycleBadge(asset);
  if (lifecycleBadge) header.appendChild(lifecycleBadge);
  header.appendChild(renderBadge(getMediaSemanticState(asset).label, getMediaSemanticState(asset).variant || 'default'));
  ...
  [
    ...
    ['Vị trí đang dùng', reference?.field || asset.fieldName || '—'],
    ['Loại', getMediaKindLabel(asset.mediaKind)],
    ['Đường dẫn kỹ thuật', asset.publicUrl || asset.publicUrlRaw || asset.storagePath || '—'],
    ['Nơi đang dùng', getResolvedMediaReferenceTarget(reference || {})?.label || asset.ownerLabel || getMediaTargetLabel(asset)],
  ].forEach(([label, value]) => appendMediaDetail(details, label, value));
...
function renderMediaLibrarySummary(summary = {}) {
  ...
  [
    ['Ảnh/video đang dùng được', summary.activeUploadLog ?? summary.uploadLog],
    ['Bản ghi upload', summary.uploadLogTotal ?? summary.uploadLog],
    ['Đã xóa', summary.deleted || 0],
    ['Tham chiếu CMS', summary.references],
    ['Chỉ thấy trong nội dung', summary.referenceOnly],
...
function renderMediaLibraryCard(asset = {}) {
  ...
  const badges = createElement('div', { className: 'cms-admin-media-card-badges' });
  badges.appendChild(renderBadge(getMediaSemanticState(asset).label, getMediaSemanticState(asset).variant || 'default'));
  badges.appendChild(renderBadge(asset.usage?.label || ADMIN_COPY.media.usageLabels.insufficient, asset.usage?.variant || 'default'));
  ...
  if (asset.hasSafePublicUrl) {
    ...
    actions.appendChild(createElement('span', { className: 'cms-admin-help-text cms-admin-media-url-warning', text: asset.publicUrlRaw ? ADMIN_COPY.media.unsafeUrl : 'Ảnh/video này chưa có đường dẫn public.' }));
  }
...
function getMediaDeleteEligibility(asset = {}) {
  ...
  if (!asset.hasUploadLogRecord) {
    return { allowed: false, reason: 'Không thể xóa: ảnh/video này chỉ thấy trong nội dung, không có bản ghi upload thật.' };
  }
...
function renderMediaDeleteGuard(asset = {}, options = {}) {
  if (!canShowMediaDeleteGuard(asset)) return createElement('div', { className: 'cms-admin-hidden' });
...
function renderMediaDeletePrepareResult(asset = {}, prepareState = null) {
  ...
  if (references.length && !eligible) {
    ...
    references.forEach((reference) => {
      ...
      const item = createElement('li');
      item.appendChild(createElement('span', { text: reference.label || reference.ownerLabel || reference.area || 'Ảnh/video được nhắc tới' }));
...
async function handlePrepareDeleteCmsMedia(asset = {}) {
  if (!asset?.id || !canShowMediaDeleteGuard(asset)) return;
...
function renderMediaReferenceDetailItem(reference = {}) {
  ...
  const detailRows = [
    ...
    [labels.field || 'Vị trí đang dùng', reference.field || '—'],
...
function renderMediaPreview(asset = {}) {
  ...
  if (!asset.hasSafePublicUrl) {
    preview.appendChild(createElement('span', { text: !asset.hasUploadLogRecord ? 'Không có preview xác minh cho đường dẫn này.' : (asset.publicUrlRaw ? ADMIN_COPY.media.unsafeUrl : ADMIN_COPY.media.previewUnavailable) }));
    return preview;
  }
  ...
  if (asset.mediaKind === 'video') {
    ...
    video.addEventListener('error', () => showMediaPreviewError(preview, asset));
  ...
  if (asset.mediaKind === 'image' || asset.mediaKind === 'poster') {
    ...
    image.addEventListener('error', () => showMediaPreviewError(preview, asset));
...
function showMediaPreviewError(preview, asset = {}) {
  if (!preview) return;
  clearNode(preview);
  preview.classList.add('has-error');
  const text = asset?.hasUploadLogRecord
    ? ADMIN_COPY.media.previewError
    : 'Không có preview xác minh. Đường dẫn này chỉ được nội dung nhắc tới và chưa khớp file trong Thư viện.';
  preview.appendChild(createElement('span', { text }));
}
...
function buildPublishCommandCenterModel(state = {}) {
  ...
  const validationErrors = validationKnown ? Object.entries(validation.errors || {}) : [];
  const validationWarnings = validationKnown ? Object.entries(validation.warnings || {}) : [];
...
function getPublishCommandNextAction(model = {}) {
  if (model.isLoadingDraft) {
    return {
      state: 'loading-content',
      label: 'Đang tải nội dung...',
      note: 'Đang tải nội dung website hiện tại vào trình duyệt. Chưa có thao tác ghi dữ liệu.',
      kind: 'disabled',
      disabledReason: 'Vui lòng chờ thao tác tải hoàn tất.',
    };
  }
  if (model.isSavingDraft) {
    return {
      state: 'saving-draft',
      label: 'Đang lưu bản chuẩn bị...',
  ...
  if (!model.validationOk) {
    return {
      state: 'content-error',
      label: 'Cần sửa nội dung trước',
      note: model.validationKnown
        ? 'Nội dung còn lỗi hoặc cảnh báo. Hãy mở nội dung cần sửa, sau đó quay lại màn này.'
        : 'Chưa có kết quả kiểm tra nội dung cho bản chuẩn bị hiện tại.',
      kind: 'blocked-content',
  ...
  if (model.access && model.access.allowed === false) {
    return {
      state: 'no-permission',
      label: 'Không đủ quyền đưa lên website',
      note: model.access.reason || 'Tài khoản hiện tại chưa đủ quyền đưa nội dung lên website.',
      kind: 'disabled',
      disabledReason: model.access.reason || 'Không đủ quyền thực hiện bước này.',
    };
  }
  return {
...
function renderPublishCommandStepper(model = {}) {
  ...
  const section = createElement('section', { className: 'cms-admin-publish-command-stepper-panel', attrs: { 'aria-label': 'Tiến trình đưa nội dung lên website' } });
  const list = createElement('ol', { className: 'cms-admin-publish-command-stepper cms-admin-publish-command-stepper-compact' });
  steps.forEach((step, index) => {
...
function getPublishCommandSteps(model = {}) {
  ...
  return [
    ...
    {
      label: 'Lưu bản chuẩn bị',
      status: !hasPreparedContent ? 'upcoming' : saved ? 'done' : 'current',
    ...
    },
    {
      ...
      status: published ? 'done' : checked ? 'current' : model.publishFailed ? 'current' : 'upcoming',
      value: published ? 'Đã đưa lên trong phiên này.' : checked ? 'Sẵn sàng xác nhận công khai.' : model.publishFailed ? 'Cần xử lý lỗi.' : 'Chưa đến bước.',
    },
  ];
}
...
function renderPublishOperationStatusCard(model = {}) {
  const card = createElement('article', { className: 'cms-admin-panel cms-admin-view-panel cms-admin-publish-command-card cms-admin-publish-command-status-card' });
  card.appendChild(renderPanelTitle('Trạng thái hiện tại', getPublishReadableStateLabel(model.nextAction?.state)));
  card.appendChild(createElement('p', {
    className: 'cms-admin-compact-copy',
  ...
  if (model.publishError) {
    card.appendChild(createElement('div', {
      className: 'cms-admin-alert cms-admin-alert-error',
      attrs: { role: 'alert' },
      text: `Chưa thể đưa nội dung lên website: ${normalizeErrorMessage(model.publishError)}`,
    }));
  }
  if (model.draftState.loadError) {
    card.appendChild(createElement('div', {
      className: 'cms-admin-alert cms-admin-alert-error',
      attrs: { role: 'alert' },
      text: `Không tải được nội dung: ${normalizeErrorMessage(model.draftState.loadError)}`,
    }));
  }
  return card;
}

function getPublishReadableStateLabel(state = '') {
  const labels = {
    ...
    'no-permission': 'Không đủ quyền',
    'ready-to-publish': 'Đã kiểm tra',
  };
...
function renderPublishLiveWebsiteCard(model = {}) {
  ...
  card.appendChild(renderPanelTitle('Website đang hoạt động', model.canonicalKnown ? 'Đã đọc' : 'Cần kiểm tra'));
  card.appendChild(createElement('p', {
    className: 'cms-admin-compact-copy',
    text: model.canonicalKnown
      ? 'Đây là nội dung đang được dùng để đối chiếu trước và sau khi đưa bản mới lên website.'
      : `Chưa đọc được nội dung website đang hoạt động: ${normalizeErrorMessage(model.canonicalError)}`,
  }));
  ...
  facts.appendChild(renderInfoTile('Phiên bản ghi nhận', model.publicVersion || 'Chưa rõ'));
...
function getPublishValidationSummary(model = {}) {
  if (!model.hasDraftJson) return 'Chưa có nội dung';
  ...
  return Number.isFinite(count) && count > 0 ? `${count} lỗi/cảnh báo` : 'Cần sửa nội dung';
}

function renderPublishValidationIssues(model = {}) {
  if (!model.hasDraftJson || model.validationOk || !model.validationKnown) return createElement('span');
  const issues = [...model.validationErrors, ...model.validationWarnings].slice(0, 4);
  if (!issues.length) return createElement('span');
  const box = createElement('div', { className: 'cms-admin-publish-validation-issues' });
  box.appendChild(createElement('strong', { text: 'Nội dung cần sửa trước khi tiếp tục' }));
  ...
  issues.forEach(([key, message]) => {
    list.appendChild(createElement('li', { text: `${formatPublishValidationKey(key)}: ${message}` }));
  });
  if ((model.validationIssueCount || 0) > issues.length) {
    list.appendChild(createElement('li', { text: `... còn ${model.validationIssueCount - issues.length} mục cần kiểm tra thêm.` }));
  }
  box.appendChild(list);
  return box;
}

function formatPublishValidationKey(key = '') {
  const text = String(key || '').replace(/^cms\./, '').replaceAll('.', ' / ').replaceAll('_', ' ');
...
function renderPublishSafetyCheckCard(model = {}) {
  ...
  card.appendChild(createElement('p', {
    className: 'cms-admin-compact-copy',
    text: 'Bước kiểm tra dùng luồng hiện hữu và không làm website đổi nội dung. Nếu đạt, bạn mới có thể xác nhận đưa bản đã lưu lên website.',
  }));
  const facts = createElement('div', { className: 'cms-admin-publish-command-facts' });
  facts.appendChild(renderInfoTile('Trạng thái', model.dryRunReady ? 'Đã kiểm tra cho phiên bản hiện tại' : 'Chưa có kết quả đạt'));
  facts.appendChild(renderInfoTile('Đang xử lý', model.isPublishingCms ? 'Có' : 'Không'));
...
function renderPublishLatestResultCard(model = {}) {
  ...
  card.appendChild(createElement('p', {
    ...
    text: model.publishSucceeded
      ? 'Đã đưa nội dung mới lên website. Bước tiếp theo là mở website và xem lịch sử phiên bản để đối chiếu.'
      : model.publishFailed
        ? 'Chưa thể đưa nội dung lên website. Xem lý do lỗi và thử lại đúng bước đang được gợi ý.'
        : 'Chưa có kết quả đưa lên website trong phiên làm việc hiện tại.',
  }));
  const facts = createElement('div', { className: 'cms-admin-publish-command-facts' });
  facts.appendChild(renderInfoTile('Phiên bản ghi nhận', model.latestPublishedVersion || '—'));
  facts.appendChild(renderInfoTile('Website đang hoạt động', model.canonicalKnown ? 'Đọc được' : 'Chưa xác nhận lại'));
  card.appendChild(facts);
  return card;
}

function renderPublishCommandRail(model = {}) {
  const rail = createElement('aside', { className: 'cms-admin-panel cms-admin-view-panel cms-admin-publish-command-rail', attrs: { 'aria-label': 'Checklist và thao tác đưa nội dung lên website' } });
  ...
  rail.appendChild(createElement('p', {
    ...
    text: 'Website đang hoạt động chỉ thay đổi sau khi bạn xác nhận đưa bản đã kiểm tra lên website.',
  }));
  return rail;
}
...
function renderPublishCommandPrimaryAction(model = {}) {
  ...
  const disabledKinds = new Set(['disabled', 'blocked-content']);
  if (action.kind === 'public-link') {
    block.appendChild(createElement('a', {
      ...
      text: action.label || 'Mở website để kiểm tra',
      href: './index.html',
      attrs: { target: '_blank', rel: 'noopener', 'aria-label': 'Mở website đang hoạt động trong tab mới để kiểm tra' },
    }));
    return block;
  }
  const button = createElement('button', {
    className: 'cms-admin-button cms-admin-button-primary cms-admin-publish-command-primary-action',
    type: 'button',
    text: action.label || 'Tiếp tục',
    attrs: { 'aria-label': action.label || 'Thực hiện bước tiếp theo' },
  });
  if (model.operationBusy || disabledKinds.has(action.kind)) {
    button.disabled = true;
    button.title = action.disabledReason || action.note || 'Chưa thể thực hiện bước này.';
  } else {
    button.addEventListener('click', () => executePublishCommandAction(action));
  }
  ...
  if (action.kind === 'blocked-content') {
    const fixButton = createElement('button', {
      className: 'cms-admin-button cms-admin-button-secondary cms-admin-button-small',
      type: 'button',
      text: 'Mở nội dung cần sửa',
      attrs: { 'aria-label': 'Mở màn Nội dung phòng 3D để sửa nội dung trước khi công khai' },
    });
    fixButton.addEventListener('click', () => switchAdminTab('staticDraft'));
...
function renderPublishCommandSecondaryActions(model = {}) {
  ...
  const websiteLink = createElement('a', {
    className: 'cms-admin-button cms-admin-button-ghost',
    href: './index.html',
    text: 'Mở website đang hoạt động',
    attrs: { target: '_blank', rel: 'noopener' },
  });
  wrap.appendChild(historyButton);
  wrap.appendChild(websiteLink);
  if (model.validationKnown && !model.validationOk) {
    const editButton = createElement('button', {
      className: 'cms-admin-button cms-admin-button-ghost',
      type: 'button',
      text: 'Mở nội dung để sửa',
      attrs: { 'aria-label': 'Mở màn Nội dung phòng 3D để sửa lỗi nội dung' },
    });
...
function handleAttachHomeMediaPickerMedia(asset = {}, descriptor = {}) {
  ...
  if (!isSelectableCmsMediaPickerAsset(asset)) {
    homeMediaPickerState.error = 'Ảnh/video này đã xóa, bị hỏng hoặc không có URL an toàn nên không thể chọn.';
    renderAdminShell();
    return;
  }
...
function getHomeMediaPickerCompatibility(asset = {}, descriptor = {}) {
  if (!isSelectableCmsMediaPickerAsset(asset)) return { allowed: false, reason: 'Ảnh/video này đã xóa, bị hỏng hoặc không có URL an toàn nên không thể chọn.' };
  if (!asset.hasSafeUrl) return { allowed: false, reason: 'Đường dẫn media không thuộc nguồn được phép. Không thể chọn media này.' };
  if (descriptor.allowedKind === 'video') {
    if (asset.mediaKind !== 'video') return { allowed: false, reason: 'Ảnh/video này không phù hợp với vị trí Trang chủ đang chọn.' };
    return { allowed: true, reason: '' };
  }
  if (!['image', 'poster'].includes(asset.mediaKind)) {
    return { allowed: false, reason: 'Ảnh/video này không phù hợp với vị trí Trang chủ đang chọn.' };
  }
  return { allowed: true, reason: '' };
}
